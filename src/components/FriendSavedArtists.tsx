...
export default function FriendSavedArtists() {
    ...
    return (
        <div className="container mx-auto p-4">
            <h2 className="text-3xl font-bold mb-8 text-center">Saved Artists</h2>

            {artists.length === 0 ? (
                <p className="text-gray-500 text-center">No saved artists found.</p>
            ) : (
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {artists.map((artist) => (
                                <Link
                                    ...
                                    className="block rounded-lg shadow-md overflow-hidden hover:shadow-xl transition bg-white"
                                >
                                ...
                                <div className="p-4">
                                    <h3 className="font-bold text-lg truncate text-center">{artist.strArtist}</h3>
                                    <p className="text-gray-500 text-center">{artist.strGenre}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
                    ...
                    <div className="flex justify-center items-center mt-12 gap-4">
                        <button
                            ...
                            disabled={page === 0}
                            className="px-4 py-2 bg-blue-500 text-white rounded disabled:bg-gray-300 transition"
                        >
                            Previous
                        </button>
