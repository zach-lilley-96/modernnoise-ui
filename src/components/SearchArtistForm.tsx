...
export default function SearchArtistForm() {
    ...
    return (
        <div className="container mx-auto p-4 max-w-3xl">
            ...
            {artists.length > 0 && (
                <div
                    className="mt-8 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-xl shadow-gray-100 dark:shadow-none overflow-hidden">
                    <div
                        className="px-6 py-4 border-b border-gray-50 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-700/50 text-gray-800 dark:text-slate-200 font-black flex justify-between items-center">
                        <span>Results</span>
                        <span
                            className="text-sm bg-gray-200 dark:bg-slate-600 px-2 py-0.5 rounded-full">{artists.length}</span>
                    </div>
                        ...
                        {artists.map((artist: ArtistDto) => (
                            ...
                            >
                                <div className="min-width-0">
                                    <div
                                        className="font-bold text-gray-900 dark:text-slate-100 text-lg group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate">
                                        {artist.strArtist}
                                    </div>
